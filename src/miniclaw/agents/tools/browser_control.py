...
_USE_SYNC_PLAYWRIGHT = (
        sys.platform == "win32" and os.environ.get("MINICLAW_RELOAD_MODE") == "1"
)
...
def _make_fresh_state(workspace_id: str, workspace_dir: str) -> dict[str, Any]:
    ...
    return {
        ...
        "cdp_url": None,
        "ignore_https_errors": True,  # 默认忽略 HTTPS 证书错误
    }
...
def _reset_browser_state(state: dict) -> None:
    ...
    state["cdp_url"] = None
    # 保持 ignore_https_errors 设置
    state["ignore_https_errors"] = True
...
def _ensure_playwright_async():
    ...
    try:
        ...
        raise ImportError(
            "Playwright not installed. Use the same Python that runs MiniClaw (e.g. "
    ...
    if exe:
        ...
        if user_data_dir:
            ...
            context = pw.chromium.launch_persistent_context(
                ...
                ignore_https_errors=state.get("ignore_https_errors", True),  # 添加 HTTPS 错误忽略
            )
...
async def _action_start(
        ...
        headed: bool = False,
        cdp_port: int = 0,
        ignore_https_errors: bool = True,  # 添加新参数
) -> ToolResponse:
    # 设置忽略 HTTPS 错误
    state["ignore_https_errors"] = ignore_https_errors
    ...
    try:
        ...
        if state.get("ignore_https_errors", True):
            msg += " (HTTPS certificate errors ignored)"
        result: dict[str, Any] = {
            "ok": True,
            "message": msg,
...
async def browser_use(  # pylint: disable=R0911,R0912
        ...
        port_min: int = 0,
        port_max: int = 0,
        ignore_https_errors: bool = True,  # 新增参数，默认忽略 HTTPS 错误
) -> ToolResponse:
    ...
    Args:
        ...
        ignore_https_errors (bool):
            When True, ignore HTTPS certificate errors (e.g., self-signed certs).
            Default True. Set to False to enforce certificate validation.
    """
