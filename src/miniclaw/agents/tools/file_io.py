...
async def tail_file(
    ...
    Args:
        file_path: Path to the file. Relative paths resolve from WORKING_DIR.
        lines: Number of lines to read from the end. Default: 100.

    Returns:
        ToolResponse: Contains the last N lines of the file or error message.
    """
    ...
    if lines <= 0:
        return ToolResponse(
            content=[
                TextBlock(
                    type="text",
                    text=f"Error: lines must be positive, got {lines}.",
                ),
            ],
        )
